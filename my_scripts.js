...
function viewStudentStats(id,toggle){
	if(toggle == 0){
		document.getElementById("id").visibility = False;
		document.getElementById("height") = 0;
	}
	else {
		document.getElementById("id").visibility = True;
		document.getElementById("height") = auto;
	}
}
...
function loadStatsPage(){
	...
	for (row_counter = 2; row_counter < table.rows.length; row_counter++){
		for (col_counter = 0; col_counter < table.rows[row_counter].cells.length;col_counter++){
			console.log(table.rows[row_counter]);
			if(col_counter == 1){
				teamname = table.rows[row_counter].cells[col_counter].innerHTML; 
			}
			if(col_counter == 2){
			...
			if(col_counter == 4){
				...
				if (cell_value2 > cell_value1){
					table.rows[row_counter].cells[col_counter].innerHTML = teamname;
					buffl_counter++
				}
			}
		}
	...
	table.innerHTML = buff_counter;
	tbale = document.getElementById("losses");
	table.innerHTML = buffl_counter;
}
